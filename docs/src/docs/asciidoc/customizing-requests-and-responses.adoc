...
Alternatively, you may want to apply the same preprocessors to every test. If you are
using MockMvc or REST Assured, you can do so by configuring the preprocessors in your
`@Before` method and using the <<documentating-your-api-parameterized-output-directories,
support for parameterized output directories>>:
