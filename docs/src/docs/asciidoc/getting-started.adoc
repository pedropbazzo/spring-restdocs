...
| Gradle
| Demonstrates the use of Spring REST docs with Spring Framework's WebTestClient.
...
<1> Add a dependency on `spring-restdocs-mockmvc` in the `test` scope. If you want to use
	...
	`spring-restdocs-webflux` or `spring-restdocs-restassured` respectively instead.
...
Spring WebFlux's `WebTestClient` or http://www.rest-assured.io[REST Assured] to make
requests to the service that you are documenting. It then produces documentation snippets
for the request and the resulting response.
...
`WebFluxRestDocumentationConfigurer` as an `ExchangeFilterFunction`.  An instance of this
class can be obtained from the static `documentationConfiguration()` method on
`org.springframework.restdocs.webflux.WebFluxRestDocumentation`.
...
`document` method on `org.springframework.restdocs.webflux.WebFluxRestDocumentation`.
