 ...
 */
public abstract class WebFluxRestDocumentation {

	private static final WebFluxRequestConverter REQUEST_CONVERTER = new WebFluxRequestConverter();

	private static final WebFluxResponseConverter RESPONSE_CONVERTER = new WebFluxResponseConverter();

	private WebFluxRestDocumentation() {
	 ...
	 */
	public static WebFluxRestDocumentationConfigurer documentationConfiguration(
			RestDocumentationContextProvider contextProvider) {
		return new WebFluxRestDocumentationConfigurer(contextProvider);
	}
	...
	public static <T extends ExchangeResult> Consumer<T> document(String identifier,
		...
		return (result) -> {
			new RestDocumentationGenerator<>(identifier, REQUEST_CONVERTER,
					RESPONSE_CONVERTER, requestPreprocessor, snippets).handle(result,
							result, retrieveConfiguration(result));
		};
	}
	...
	public static <T extends ExchangeResult> Consumer<T> document(String identifier,
		...
		return (result) -> {
			new RestDocumentationGenerator<>(identifier, REQUEST_CONVERTER,
					RESPONSE_CONVERTER, responsePreprocessor, snippets).handle(result,
							result, retrieveConfiguration(result));
		};
	}
	...
	public static <T extends ExchangeResult> Consumer<T> document(String identifier,
		...
		return (result) -> {
			new RestDocumentationGenerator<>(identifier, REQUEST_CONVERTER,
					RESPONSE_CONVERTER, requestPreprocessor, responsePreprocessor,
					snippets).handle(result, result, retrieveConfiguration(result));
		};
	...
	private static Map<String, Object> retrieveConfiguration(ExchangeResult result) {
		Map<String, Object> configuration = new HashMap<>(
				WebFluxRestDocumentationConfigurer
